      ...
      <>
        ...
        <main className="pt-24 pb-20 min-h-screen bg-cream">
          <div className="text-center py-20">
            <p className="text-navy/40 font-body">Loading...</p>
          </div>
        </main>
        <Footer />
      </>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-navy/40 text-sm font-body mt-1">{email}</p>
            </div>
            <button onClick={signOut} className="text-xs font-body text-navy/40 hover:text-navy underline">Sign out</button>
          </div>

          {loading ? (
            <p className="text-navy/40 font-body">Loading appointments...</p>
          ) : (
            <>
              <section className="mb-10">
                ...
                {upcoming.length === 0 ? (
                  <div className="bg-white border border-navy/10 p-8 text-center">
                    <p className="text-navy/40 font-body text-sm mb-4">No upcoming appointments.</p>
              ...
              <section>
                ...
                {past.length === 0 ? (
                  <p className="text-navy/40 text-sm font-body">No past appointments.</p>
                ) : (
                    ...
                    {past.slice(0, 20).map((a) => (
                      <div key={a.id} className="px-5 py-3 flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-navy/40 text-xs font-body">with {a.stylists?.name || "Stylist"} · {formatDate(a.date)}</p>
                        </div>
