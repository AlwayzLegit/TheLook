...
export default function Hero() {
  ...
  // Gate motion behind a mount flag. Two distinct mismatches lived
  // here before:
  //   1. Parallax y/opacity (scroll-driven MotionValues vs. fallback
  //      strings) — handled by the y / opacity ternaries below.
  //   2. The reveal animations (initial={{opacity:0}} animate={{opacity:1}})
  //      — framer-motion serialises these to the *animate* state on SSR
  //      but the client's first render uses *initial*, producing different
  //      inline-style strings. React #418 hydration mismatch.
  // MotionConfig isStatic={!mounted} fixes #2: every motion.* element
  // inside renders with no animation work until hydration completes,
  // so SSR and client first paint produce identical markup.
