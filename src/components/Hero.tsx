...
//
//   1. Parallax y/opacity (scroll-driven MotionValues vs. fallback strings):
//      gated by the `mounted` ternaries below. SSR + first client paint
//      both render with the static "0%" / 1 fallback; after mount they
//      upgrade to live MotionValues.
//
//   2. Reveal animations (initial={{opacity:0}} animate={{opacity:1}}):
//      framer-motion's SSR pass writes the *animate* state to the inline
//      style, while the client's first render uses *initial* — different
//      style attributes → React #418. Earlier attempt wrapped this in
//      <MotionConfig isStatic={!mounted}>, but isStatic freezes motion
//      components AT their initial state, leaving the entire Hero stuck
//      invisible at opacity 0 on first paint. Final fix: pass
//      `initial={false}` while !mounted so the SSR + first client render
//      both lock to the *animate* target (visible). After mount the
//      animation can play; we also pass an actual initial so reveals
//      still happen on subsequent client navigations.
//
//   3. The trailing scroll-indicator dot pulses forever — it has no
//      `initial` prop so SSR + client always agree. Left untouched.
...
export default function Hero() {
  ...
  // While !mounted, force initial=false on every reveal so the SSR
  // markup matches the client's first render. Once hydrated, real
  // initial values are passed (animation plays only on client-side
  // navigation, not on the first paint we just hydrated).
  const ssrSafeInitial = <T,>(initial: T) => (mounted ? initial : (false as const));
  ...
  return (
    ...
    >
      ...
      <motion.div style={{ opacity }} className="relative z-10 text-center px-6 max-w-4xl mx-auto">
        {/* Badge */}
        <motion.div
          initial={ssrSafeInitial({ opacity: 0 })}
          animate={{ opacity: 1 }}
        ...
        <motion.h1
          initial={ssrSafeInitial({ opacity: 0, y: 15 })}
        ...
        {/* Ornamental divider */}
        <motion.div
          initial={ssrSafeInitial({ scaleX: 0 })}
          animate={{ scaleX: 1 }}
        ...
        {/* CTAs */}
        <motion.div
          initial={ssrSafeInitial({ opacity: 0, y: 10 })}
          animate={{ opacity: 1, y: 0 }}
