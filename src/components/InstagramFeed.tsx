...
export default function InstagramFeed() {
  return (
    <section className="py-28 md:py-36 bg-white relative overflow-hidden">
      {/* Background accent */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(196,162,101,0.04)_0%,transparent_50%)]" />
          ...
          {posts.map((post, index) => (
            <AnimatedSection key={index} delay={index * 0.05}>
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-navy/70 via-navy/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-end justify-center pb-6">
                  <div className="flex items-center gap-2 text-white translate-y-3 group-hover:translate-y-0 transition-transform duration-500">
                    <svg
                      className="w-5 h-5"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    ...
                    <span className="text-xs font-body tracking-wider">View</span>
                  </div>
                </div>
              </a>
            </AnimatedSection>
